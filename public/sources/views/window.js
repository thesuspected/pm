...
import {_to_array, bind, clone, delay, extend, toNode, uid} from "../webix/helpers";
...
const api = {
    ...
    _hiding_process: function () {
        if (this._settings.position == "top") {
            animate(this._viewobj, {
                type: "slide", x: 0, y: -(this._content_height + 20), duration: 300,
    ...
    head_setter: function (value) {
        ...
        if (text && this.config.close) {
            value = {
                padding: {left: $active.inputHeight + 2, right: 2}, cols: [
                    value,
                    {
