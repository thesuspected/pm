...
func (m *EmployeeMapper) SelectGroup(id int, db *sql.DB) (employees []*resources.Employee, err error) {
	rows, err := db.Query(
		`SELECT e.c_id, e.c_last_name, e.c_first_name, e.c_patronymic, e.c_date, e.c_email, e.c_img_src, u.c_login, p.c_name, g.c_name
				FROM t_employees AS e, t_users AS u, t_ref_positions AS p, t_groups AS g
				WHERE e.fk_user = u.c_id and e.fk_position = p.c_id and e.fk_group = $1
				ORDER BY e.c_id`, id)
	if err != nil {
		return employees, err
	}
